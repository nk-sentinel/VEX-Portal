...
class TestKev:
    ...
    def test_no_unanswerable_state_is_possible(self) -> None:
        """Flagged gap: Tier3Signals.kev is a plain bool defaulting to False
        ('nothing known' reads as 'no blocker implied'), not Optional, so a
        KEV lookup that never ran is indistinguishable from one that
        confirmed 'not KEV'. Documented here rather than left implicit."""
        rule = Kev()
...
class TestNoFixAvailable:
    ...
    def test_no_unanswerable_state_is_possible(self) -> None:
        """Flagged gap, mirroring t3-kev: Tier3Signals.fix_available is a
        plain bool defaulting to True, not Optional, so a remediation lookup
        that never ran is indistinguishable from one that confirmed a fix
        exists."""
        rule = NoFixAvailable()
        result = rule.evaluate(PACK, COMPONENT, Tier3Signals())
        assert result.verdict is RuleVerdict.NOT_SATISFIED
...
class TestNeverClears:
    ...
    def test_no_combination_of_tier3_signals_ever_yields_not_affected(self) -> None:
        ...
        fix_available_values = (True, False)
